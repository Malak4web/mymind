...
- In `src/App.vue` (line 485):
  ...
  ```
- Column span assignments across the 3 layout children on `xl`:
...
- **Calculated Grid Math Sum per Combination on `xl`**:
  - Combination 1 (`isSidebarCollapsed = false`, `isInspectorOpen = false`):
    Sidebar (3) + Main Canvas (9) + Inspector (0, hidden) = **12 columns**.
  - Combination 2 (`isSidebarCollapsed = true`, `isInspectorOpen = false`):
    Sidebar (1) + Main Canvas (11) + Inspector (0, hidden) = **12 columns**.
  - Combination 3 (`isSidebarCollapsed = false`, `isInspectorOpen = true`):
    Sidebar (3) + Main Canvas (6) + Inspector (3) = **12 columns**.
  - Combination 4 (`isSidebarCollapsed = true`, `isInspectorOpen = true`):
...
- **Null / Missing Attachments**:
  ...
  - **Failure Mode 1**: In `src/store.js` line 770 (`uploadFileToTask`), `task.attachments.push(tempFile)` is called directly. If `task.attachments` is `null` or `undefined`, calling `uploadFileToTask` throws `TypeError: Cannot read properties of null (reading 'push')`.
  - **Failure Mode 2**: In `src/components/QuickInspector.vue` line 282: `<span class="font-bold truncate text-slate-700 dark:text-slate-300 max-w-[180px]">{{ file.name }}</span>`. If an element inside `activeTask.attachments` is `null` (e.g. `attachments: [null]`), template rendering throws `TypeError: Cannot read properties of null (reading 'name')`.
...
- **Failure Mode 3**: Lack of `try...catch` wrapper around `localStorage.getItem` or `localStorage.setItem` for restricted storage contexts (e.g., cross-origin iframes or disabled cookies/storage).
...
- `npx vitest run`: Exited 0. Passed 69 tests across 9 test files, including our dedicated stress harness `src/__tests__/m2_layout_stress.spec.js`.
...
1. **Grid Math Verification**:
   - The desktop grid relies on CSS Grid with 12 columns (`xl:grid-cols-12`).
   - For all 4 states of `(isSidebarCollapsed, isInspectorOpen)`, the sum of child grid spans (`col-span`) equals exactly 12.
   - When the Inspector is closed (`isInspectorOpen = false`), its container receives class `hidden` (`display: none`), occupying 0 grid columns, while Main Workspace dynamically expands to fill remaining space (9 cols when sidebar is expanded, 11 cols when sidebar is collapsed).
   - Conclusion: Layout grid math is mathematically consistent and correctly responsive.

2. **QuickInspector Edge Case Vulnerabilities**:
   - In `store.uploadFileToTask`, `task.attachments` is mutated via `.push()`. If a task is created or returned from backend with `attachments: null` (instead of `[]`), `.push()` throws an unhandled `TypeError`.
   - In `QuickInspector.vue`, template binding `file.name` lacks optional chaining (`file?.name`). If an attachments array contains `null`, Vue component render throws an unhandled render function error.
   - Conclusion: Primitive fields (title, description, dates, memberIds) handle nulls safely, but array operations on `attachments` require defensive initialization and optional chaining.
...
- **CSS Visual Rendering**: Automated tests verify HTML class attributes and Vue state bindings; browser visual pixel layout was verified via static grid math analysis rather than headless canvas screenshotting.
- **Backend API Sync for Attachments**: `uploadFileToTask` in `store.js` simulates upload progress before hitting the backend endpoint `/tasks/{id}/attachments`.
...
- **Build & Tests**: Production build (`npm run build`) builds cleanly with zero errors. All 69 Vitest tests pass cleanly.
...
npx vitest run
```
*Expected Result*: All 9 test suites (69 tests including `src/__tests__/m2_layout_stress.spec.js`) pass cleanly with exit code 0.
