...
// Mock simple reactive store implementation matching store.js
const storeModule = require(path.join(__dirname, '../../src/store.js'));
const store = storeModule.store;

assert(Array.isArray(store.habits) && store.habits.length >= 4, 'Default 4 habits loaded into store');

// Clear local storage and test persistence actions
const initialHabitCount = store.habits.length;

// Test addHabit
const addedHabit = store.addHabit({
  title: 'عادة اختبارية جديدة',
  category: 'تطوير ذات',
  icon: '🚀',
...
assert(addedHabit && addedHabit.id, 'addHabit returns newly created habit with ID');
assert(store.habits.length === initialHabitCount + 1, 'Store habits array incremented by 1');

const savedString = localStorage.getItem('mymind_habits');
assert(savedString !== null, 'mymind_habits saved in localStorage');
const parsedSaved = JSON.parse(savedString);
assert(parsedSaved.some(h => h.id === addedHabit.id && h.title === 'عادة اختبارية جديدة'), 'Newly added habit verified in localStorage JSON');

// Test toggleHabitLog
const testDateStr = '2026-07-30';
store.toggleHabitLog(addedHabit.id, testDateStr);
const updatedSavedString = localStorage.getItem('mymind_habits');
const updatedParsed = JSON.parse(updatedSavedString);
const savedHabit = updatedParsed.find(h => h.id === addedHabit.id);

assert(savedHabit.logs[testDateStr]?.completed === true, 'toggleHabitLog sets completed: true and persists to localStorage');

// Untoggle
store.toggleHabitLog(addedHabit.id, testDateStr);
const untoggledParsed = JSON.parse(localStorage.getItem('mymind_habits'));
assert(untoggledParsed.find(h => h.id === addedHabit.id).logs[testDateStr]?.completed === false, 'toggleHabitLog toggles back to completed: false and persists to localStorage');

// Test addHabitChecklistItem & toggleHabitChecklistItem
const item = store.addHabitChecklistItem(addedHabit.id, 'خطوة فرعية أولى');
assert(item && item.title === 'خطوة فرعية أولى', 'addHabitChecklistItem returns item');

let storedAfterChecklist = JSON.parse(localStorage.getItem('mymind_habits')).find(h => h.id === addedHabit.id);
assert(storedAfterChecklist.checklist.length === 1 && storedAfterChecklist.checklist[0].completed === false, 'Checklist item persisted to localStorage');

store.toggleHabitChecklistItem(addedHabit.id, item.id);
storedAfterChecklist = JSON.parse(localStorage.getItem('mymind_habits')).find(h => h.id === addedHabit.id);
assert(storedAfterChecklist.checklist[0].completed === true, 'Checklist toggle persisted to localStorage');

store.deleteHabitChecklistItem(addedHabit.id, item.id);
storedAfterChecklist = JSON.parse(localStorage.getItem('mymind_habits')).find(h => h.id === addedHabit.id);
assert(storedAfterChecklist.checklist.length === 0, 'Checklist item deletion persisted to localStorage');

// Test addHabitNote & deleteHabitNote
const note = store.addHabitNote(addedHabit.id, '🤩 | انطباع إيجابي مميز', testDateStr);
assert(note && note.content.includes('انطباع إيجابي'), 'addHabitNote returns note object');

let storedAfterNote = JSON.parse(localStorage.getItem('mymind_habits')).find(h => h.id === addedHabit.id);
assert(storedAfterNote.notesList.length === 1 && storedAfterNote.notesList[0].content.includes('🤩'), 'Note entry persisted to localStorage');

store.deleteHabitNote(addedHabit.id, note.id);
storedAfterNote = JSON.parse(localStorage.getItem('mymind_habits')).find(h => h.id === addedHabit.id);
assert(storedAfterNote.notesList.length === 0, 'Note deletion persisted to localStorage');

// Test deleteHabit
store.deleteHabit(addedHabit.id);
const storedAfterDelete = JSON.parse(localStorage.getItem('mymind_habits'));
assert(!storedAfterDelete.some(h => h.id === addedHabit.id), 'deleteHabit removes habit from store & localStorage');
