...
<template>
  ...
  <div 
    ...
    :class="[
      store.theme, 
      'min-h-screen overflow-x-hidden transition-colors duration-500 text-slate-800 dark:text-slate-100 font-sans antialiased selection:bg-violet-500/10 selection:text-violet-600 bg-slate-50/50 dark:bg-slate-950'
    ]"
  >
    ...
    <main class="max-w-full w-full px-4 md:px-8 py-6 pb-28 md:pb-8 transition-all duration-500 relative z-10">
      ...
      <div v-if="store.isFocusMode" class="flex justify-center mb-8 animate-fade-in">
        <button 
          ...
          class="bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-700 hover:to-indigo-700 text-white font-extrabold px-6 py-3 rounded-full text-xs shadow-lg shadow-violet-500/25 cursor-pointer flex items-center space-x-2 transition hover:scale-[1.03]"
        >
      ...
      <div :class="[
        ...
        <!-- Task inspector.
             It used to be `hidden xl:block`, so on every phone, tablet and
             sub-1280px laptop a single tap on a task set the state and
             rendered nothing at all. Below xl it now opens as a bottom sheet;
             at xl and above it stays the docked third column. -->
        <template v-if="!store.isFocusMode && store.activeView !== 'settings' && store.activeView !== 'routines'">
          <Transition name="fade">
            <div
              v-if="store.isInspectorOpen"
              @click="store.closeTaskInspector()"
              class="xl:hidden fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-overlay"
              aria-hidden="true"
            ></div>
          </Transition>
          ...
          <Transition name="sheet">
            <div
              v-if="store.isInspectorOpen"
              class="fixed inset-x-0 bottom-0 z-sheet max-h-[88vh] overflow-y-auto
                     xl:static xl:inset-auto xl:max-h-none xl:overflow-visible xl:col-span-3"
              role="dialog"
              aria-modal="true"
        ...
        <div class="relative z-10 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-xl shadow-2xl overflow-hidden p-4 space-y-4">
          <div class="flex items-center gap-2 border-b border-slate-100 dark:border-slate-800 pb-3">
            ...
            <input 
              ...
              class="w-full bg-transparent text-sm font-semibold text-slate-900 dark:text-slate-100 focus:outline-none"
              autofocus
            />
          ...
          <div class="max-h-80 overflow-y-auto space-y-3">
            ...
            <div v-if="quickSearchResults.projects.length > 0">
              <span class="text-[10px] font-extrabold text-slate-400 block mb-1">المشاريع</span>
              <div 
            ...
            <div v-if="quickSearchResults.tasks.length > 0">
              <span class="text-[10px] font-extrabold text-slate-400 block mb-1">المهام</span>
              <div 
      ...
      <div v-if="showMobileProjectsSheet" class="md:hidden fixed inset-0 z-50 flex items-end justify-center" dir="rtl">
        ...
        >
          ...
          <div
            ...
            class="shrink-0 flex justify-center py-2.5 cursor-grab touch-none"
            aria-hidden="true"
          >
            <div class="w-12 h-1.5 bg-slate-300 dark:bg-slate-700 rounded-full"></div>
          </div>
        ...
        >
          ...
          <div class="grid grid-cols-2 gap-3 mb-2">
            <button 
              ...
              class="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200/60 dark:border-slate-800 text-right flex items-center justify-between cursor-pointer"
            >
