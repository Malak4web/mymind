...
<template>
  <div class="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 font-sans selection:bg-violet-500/10 selection:text-violet-600 transition-colors duration-500 relative overflow-hidden" dir="rtl">
    ...
    <header class="sticky top-0 z-30 bg-white/70 dark:bg-slate-950/70 backdrop-blur-md border-b border-slate-200/50 dark:border-slate-850/80 transition-all duration-300">
      <div class="max-w-7xl mx-auto px-6 md:px-12 py-4 flex items-center justify-between">
        
        <!-- Logo -->
        <div class="flex items-center gap-3">
          <div class="w-9 h-9 rounded-xl bg-gradient-to-tr from-violet-600 to-indigo-650 flex items-center justify-center shadow-md shadow-violet-500/10">
          ...
          <span class="text-lg font-extrabold text-slate-850 dark:text-slate-50 flex items-center gap-1">
            MyMind
            <span class="px-1.5 py-0.5 rounded bg-gradient-to-r from-violet-600 to-indigo-650 text-[9px] font-extrabold text-white">برو</span>
          </span>
        </div>
        ...
        <div class="flex items-center gap-4">
          <button 
            ...
            class="p-2 border border-slate-200 dark:border-slate-800 hover:border-slate-350 dark:hover:border-slate-700 bg-white dark:bg-slate-900 text-slate-500 dark:text-slate-400 rounded-xl transition cursor-pointer hover:shadow-sm"
    ...
    <main class="relative z-10 max-w-7xl mx-auto px-6 md:px-12 py-12 md:py-24 space-y-32">
      ...
      <section id="hero" class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        ...
        <div class="lg:col-span-6 space-y-6 text-right">
          ...
          <h1 class="text-4xl md:text-5xl lg:text-6xl font-extrabold text-slate-850 dark:text-slate-50 leading-tight">
          ...
          <div class="flex items-center gap-4 flex-wrap pt-2">
            ...
            <button 
              ...
              class="border border-slate-200 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-900 text-slate-655 dark:text-slate-300 font-bold text-sm px-7 py-3.5 rounded-2xl transition cursor-pointer"
            >
              شاهد الميزات
            </button>
          </div>
          ...
          <!-- Quick statistics -->
          <div class="grid grid-cols-3 gap-6 border-t border-slate-200/60 dark:border-slate-850 pt-8 mt-4">
            <div>
              <span class="block text-2xl font-extrabold text-slate-850 dark:text-slate-50">99.9%</span>
              <span class="text-[10px] text-slate-400 font-bold uppercase tracking-wider block mt-1">زمن العمل النشط</span>
            </div>
            <div>
              <span class="block text-2xl font-extrabold text-slate-850 dark:text-slate-50">MySQL</span>
              <span class="text-[10px] text-slate-400 font-bold uppercase tracking-wider block mt-1">قاعدة بيانات محلية</span>
            </div>
            <div>
              <span class="block text-2xl font-extrabold text-slate-850 dark:text-slate-50">Zero</span>
              <span class="text-[10px] text-slate-400 font-bold uppercase tracking-wider block mt-1">تكلفة الإعداد الأولي</span>
            </div>
          </div>
        </div>
        ...
        <div class="lg:col-span-6 flex justify-center relative">
          ...
          <div class="relative w-full max-w-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl p-6 space-y-6 z-10 transform hover:scale-[1.02] transition-transform duration-300 select-none">
            ...
            <!-- Mock Header Bar -->
            <div class="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-850 flex-row-reverse">
            ...
            <div class="grid grid-cols-3 gap-2.5">
              <!-- Columns -->
              <div v-for="col in ['بانتظار البدء', 'قيد العمل', 'مكتمل']" :key="col" class="space-y-2.5 bg-slate-50/50 dark:bg-slate-955/30 border border-slate-200/50 dark:border-slate-850 p-2.5 rounded-xl min-h-[220px]">
                <span class="block text-[9px] font-extrabold text-slate-400 text-center uppercase tracking-wide">{{ col }}</span>
                ...
                >
                  <p class="text-slate-850 dark:text-slate-200 leading-snug">{{ task.title }}</p>
                  <span class="text-[8px] text-violet-500 font-bold block mt-1.5">انقر للنقل ➔</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <section id="features" class="space-y-12">
        <div class="text-center space-y-2 max-w-2xl mx-auto">
          <h2 class="text-3xl font-extrabold text-slate-850 dark:text-slate-100">صُمم لرفع كفاءة تفكيرك وعملك</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div 
            ...
            :class="[
              ...
              'p-6 bg-white dark:bg-slate-900 border rounded-3xl transition-all duration-300 text-right space-y-4 hover:border-slate-350 dark:hover:border-slate-700'
            ]"
          >
            <div class="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-850 flex items-center justify-center text-lg">
              {{ f.icon }}
            </div>
            <h3 class="text-sm font-extrabold text-slate-850 dark:text-slate-100">{{ f.title }}</h3>
      ...
      <section id="tech" class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        ...
        <div class="lg:col-span-6 space-y-4 text-right">
          ...
          <div class="bg-gradient-to-tr from-slate-50/60 to-white/60 dark:from-slate-950/20 dark:to-slate-900/20 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 md:p-8 space-y-4">
            <h3 class="text-base font-extrabold text-slate-855 dark:text-slate-100">هندسة النظام المحلية المستقرة</h3>
            ...
            <div class="flex items-center justify-between border-t border-slate-200/60 dark:border-slate-800 pt-4 mt-2 font-mono text-[10px] text-slate-400">
              <div class="bg-slate-100 dark:bg-slate-800 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-750">Vue 3 Frontend</div>
              <div class="text-violet-500">➔</div>
              <div class="bg-slate-100 dark:bg-slate-800 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-750">Laravel API</div>
              <div class="text-violet-500">➔</div>
              <div class="bg-slate-100 dark:bg-slate-800 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-750">MySQL Local DB</div>
            </div>
          </div>
        </div>
        ...
        <div class="lg:col-span-6 space-y-6 text-right">
          <h2 class="text-3xl font-extrabold text-slate-850 dark:text-slate-100">أداء فوري واستجابة فورية</h2>
          ...
          <div class="p-4 bg-emerald-500/[0.03] border border-emerald-500/10 rounded-2xl flex items-start gap-3 flex-row-reverse text-right">
            ...
            <div class="space-y-1">
              <h5 class="text-xs font-extrabold text-slate-855 dark:text-slate-100">دعم كامل للغة العربية</h5>
              <p class="text-[11px] text-slate-455">الواجهات مصممة من الصفر باتجاه RTL ومتوافقة مع الخط العربي الجميل Tajawal.</p>
            </div>
          </div>
        </div>
      ...
      <section id="pricing" class="space-y-12">
        <div class="text-center space-y-2 max-w-2xl mx-auto">
          <h2 class="text-3xl font-extrabold text-slate-855 dark:text-slate-100">خطط تناسب تطلعاتك</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-3xl mx-auto">
          ...
          <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-8 space-y-6 text-right relative flex flex-col justify-between">
            <div class="space-y-4">
              ...
              <p class="text-xs text-slate-455 leading-relaxed font-semibold">تخطيط مبدئي للحد الأدنى من المهام والمشاريع دون مميزات برو المتقدمة.</p>
              
              <div class="text-3xl font-extrabold text-slate-850 dark:text-slate-50">
                0$ <span class="text-xs font-semibold text-slate-400">/ للأبد</span>
              </div>

              <div class="border-t border-slate-100 dark:border-slate-850 pt-4 space-y-2.5 text-xs text-slate-500 dark:text-slate-400">
            ...
            <button @click="isLoginModalOpen = true" class="w-full mt-6 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-750 text-slate-700 dark:text-slate-300 font-bold py-2.5 rounded-xl text-xs transition cursor-pointer">ابدأ الآن</button>
          </div>
          ...
          <div class="bg-white dark:bg-slate-900 border-2 border-violet-500 dark:border-violet-600 rounded-3xl p-8 space-y-6 text-right relative flex flex-col justify-between shadow-xl">
            <!-- Badge -->
            <div class="absolute -top-3.5 right-6 bg-violet-600 text-white text-[9px] font-extrabold px-3 py-1 rounded-full uppercase tracking-wider">الخيار الأكثر تميزاً</div>
            
            <div class="space-y-4">
              <span class="text-xs font-bold text-violet-600 dark:text-violet-400 block">نسخة البروفيسور</span>
              <h3 class="text-xl font-extrabold text-slate-800 dark:text-slate-100">عقلي برو</h3>
              <p class="text-xs text-slate-455 leading-relaxed font-semibold">كافة الخصائص المتقدمة والمميزات التفاعلية لبناء القوالب والملفات بلا حدود.</p>
    ...
    <!-- Footer -->
    <footer class="border-t border-slate-200/60 dark:border-slate-850/80 bg-white/30 dark:bg-slate-955/10 py-12 relative z-10">
      <div class="max-w-7xl mx-auto px-6 md:px-12 flex flex-col md:flex-row items-center justify-between gap-6 text-center">
        ...
        <span class="text-sm font-extrabold text-slate-500 flex items-center gap-1.5">
          MyMind
          <span class="px-1.5 py-0.5 rounded bg-slate-200 dark:bg-slate-800 text-[8px] font-extrabold text-slate-500">برو</span>
        </span>
        
        <!-- Copyright -->
        <p class="text-xs text-slate-400 font-semibold">© 2026 عقلي برو. جميع الحقوق محفوظة لمحرك التطوير المحلي.</p>
      ...
      <div v-if="isLoginModalOpen" class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" dir="rtl">
        ...
        <div class="relative w-full max-w-md bg-white dark:bg-slate-900 border-t sm:border border-slate-200 dark:border-slate-800 rounded-t-3xl sm:rounded-3xl p-6 sm:p-8 max-h-[90vh] overflow-y-auto shadow-2xl space-y-6 z-10 transition-all duration-300 transform scale-100">
          ...
          <button 
            ...
            class="absolute top-4 left-4 text-slate-400 hover:text-slate-655 dark:hover:text-slate-200 p-1.5 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition cursor-pointer"
          >
          ...
          <div class="flex flex-col items-center text-center space-y-2.5">
            <div class="w-12 h-12 rounded-xl bg-gradient-to-tr from-violet-600 to-indigo-650 flex items-center justify-center shadow-lg shadow-violet-500/15">
            ...
            <div>
              <h4 class="text-xl font-extrabold text-slate-850 dark:text-slate-50 flex items-center justify-center gap-1.5">
                MyMind
                <span class="px-1.5 py-0.5 rounded-lg bg-gradient-to-r from-violet-650 to-indigo-650 text-[10px] font-extrabold text-white">برو</span>
              </h4>
          ...
          <form @submit.prevent="handleLogin" class="space-y-4">
            <div>
              ...
              <input 
                ...
                class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-805 rounded-xl px-4 py-2.5 text-sm text-slate-850 dark:text-slate-200 focus:outline-none focus:border-violet-500 transition"
              />
            </div>
            ...
            <div>
              ...
              <input 
                ...
                required
                placeholder="******"
                class="w-full bg-slate-50 dark:bg-slate-955 border border-slate-200 dark:border-slate-805 rounded-xl px-4 py-2.5 text-sm text-slate-850 dark:text-slate-200 focus:outline-none focus:border-violet-500 transition"
              />
            </div>
          ...
          <div class="pt-4 border-t border-slate-100 dark:border-slate-800 text-xs text-slate-455 space-y-2 text-right">
            <span class="font-bold text-slate-655 dark:text-slate-350 block">حسابات تجريبية للوصول (MySQL Seeds):</span>
            <div class="space-y-1">
              <div class="flex justify-between font-mono text-[10px] flex-row-reverse"><span class="font-bold">مدير:</span> <span>khaled@mymind.com | password123</span></div>
              <div class="flex justify-between font-mono text-[10px] flex-row-reverse"><span class="font-bold">عضو:</span> <span>sara@mymind.com | password123</span></div>
              <div class="flex justify-between font-mono text-[10px] flex-row-reverse"><span class="font-bold">مشاهد:</span> <span>ahmed@mymind.com | password123</span></div>
            </div>
          </div>
